...
import { closeUpdateModal } from "../../features/stockSlice";

const UpdateStockModal = () => {
  const [symbol, setSymbol] = useState("");
  const dispatch = useDispatch();
  const { isUpdateModalOpen, selectedStock, loading } = useSelector(
    (state) => state.stock
  );
  const { updateStock } = useStockCall();
  ...
  const handleSubmit = async (e) => {
    ...
    await updateStock(selectedStock._id, updatedStock);
    setSymbol("");
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="w-full max-w-md p-6 bg-white rounded-lg shadow-xl">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-[#161616]">Update Stock</h3>
          <button
            onClick={handleClose}
